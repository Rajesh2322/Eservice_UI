...
export class DigitalServicesComponent {
  ...
  role: string = '';
  showNewServiceForm: boolean = true;
  msg: string = '';
  newService: any = {};
  ...
  viewUserProfileDetails() {
   ...
   logout() {
    ...
    this.http.post<Response>(this.serviceUrl + 'user/logOut', {}).subscribe(
      response => {
        if (response.statusCodeValue === 200) {
  ...
  submitNewServiceForm(newServiceForm: any) {
   if(newServiceForm && newServiceForm.invalid){
    return;
   }
const  newService: any = {
  serviceName: this.newService.name,
  createdBy: this.sharedDataService.getUsername()
};
const bearerToken = localStorage.getItem('token');
    ...
    this.http.post<Response>(this.serviceUrl + 'digitalServices/create', newService, { headers }).subscribe(
      (response) => {
        if (response.statusCodeValue === 201) {
          this.msg = response.body.msg + "----" + response.body.data;
          this.displaySuccessMessage(this.msg);
          this.resetForm(newService);
          this.hideNewServiceForm();
